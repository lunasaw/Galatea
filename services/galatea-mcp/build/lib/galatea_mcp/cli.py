...
class PlatformConfig(Strict):
    trainer: Target
    evaluator: Target
    ray_topology: Literal['separate_endpoints', 'shared_serial_endpoint'] = 'separate_endpoints'
    credential_mode: Literal['separate_role_credentials', 'shared_read_only'] = 'separate_role_credentials'
    tracking_uri: str
    s3_endpoint: str
    ...
    def validate_ray_topology(self):
        if self.ray_topology == 'shared_serial_endpoint':
            if (self.trainer.address != self.evaluator.address
                    or self.trainer.expected_head_id != self.evaluator.expected_head_id):
                raise ValueError(
                    'shared_serial_endpoint requires the same Ray address and expected head ID'
                )
            if self.credential_mode == 'shared_read_only':
                if not self.trainer.env_refs or self.trainer.env_refs != self.evaluator.env_refs:
                    raise ValueError(
                        'shared_read_only requires the same non-empty credential references for both roles'
                    )
            else:
                trainer_refs = set(self.trainer.env_refs.values())
                evaluator_refs = set(self.evaluator.env_refs.values())
                if not trainer_refs or not evaluator_refs or trainer_refs & evaluator_refs:
                    raise ValueError('separate_role_credentials requires independent trainer/evaluator credentials')
        elif self.trainer.address == self.evaluator.address:
            raise ValueError('separate_endpoints requires distinct Ray addresses')
        return self
...
def official_backends(config: PlatformConfig):
    ...
    signer = BindingSigner(key, tracking_uri=config.tracking_uri, role_env={},
                           ...
                           ray_topology=config.ray_topology,
    ...
    ray = RayBackend(clients, expected_heads={n:t.expected_head_id for n,t in targets.items()},
                     endpoint_mode=config.ray_topology,
    ...
    try:
        ...
        if args.command == 'preflight':
            ...
            print(json.dumps({'status':'platform-reachable','cluster_identity':ray.cluster_id,
                              'ray_topology':deployment.platform.ray_topology,
                              'credential_mode':deployment.platform.credential_mode,
