...
from ray_handwritten_digits.integrity import (  # noqa: E402
...
class IntegrityTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.dataset = SimpleNamespace(
            split_frame=lambda name: pd.DataFrame(
                {
                    "relative_path": {
                        "training": ["digit_0/a.jpg", "digit_1/b.jpg"],
                        "validation": ["digit_0/c.jpg"],
                        "test": ["digit_1/d.jpg"],
                    }[name],
                    "sha256": {
                        "training": ["hash-a", "hash-b"],
                        "validation": ["hash-c"],
                        "test": ["hash-d"],
                    }[name],
                }
    ...
    def test_report_is_deterministic_and_contains_opaque_context_parity(self) -> None:
        ...
        self.assertEqual(
            first["preprocessing"]["context_identity_tuples"]["fit"]["base_preprocessing_identity_sha256"],
            first["preprocessing"]["context_identity_tuples"]["test"]["base_preprocessing_identity_sha256"],
        )
        self.assertEqual(64, len(first["integrity_digest"]))
        self.assertEqual(first["preprocessing_digest"], first["preprocessing"]["digest"])
        self.assertEqual(
            first["migration_contamination_digest"],
            first["migration"]["contamination"]["digest"],
        )

    def test_trial_integrity_does_not_use_test_context_or_content(self) -> None:
        ...
        self.assertNotIn("test∩training", boundaries["overlap"]["pairwise"])
    ...
    def test_split_grouping_keeps_duplicate_content_in_one_split(self) -> None:
        from ray_handwritten_digits.data import _assign_grouped_splits
    ...
    def test_champion_integrity_declares_test_boundary(self) -> None:
        ...
        self.assertTrue(boundaries["test_use"]["allowed"])
        self.assertEqual("passed", report["migration"]["contamination"]["status"])
