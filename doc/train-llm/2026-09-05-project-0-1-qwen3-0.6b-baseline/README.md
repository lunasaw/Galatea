...
开放式聊天的质量比较不使用单一准确率。完成本阶段的运行基线后，按通用的
[`开放式聊天微调评测方案`](../fine-tuning-evaluation-protocol.md)比较 Base、Prompt-only 和 LoRA；
本阶段的 `baseline_report.json` 只作为运行性能基线，不是质量或训练收益证据。
