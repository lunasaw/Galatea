...
from ray_handwritten_digits.data import PreparedDataset
...
def log_run_inputs(
    ...
    for split_name in ("training", "validation", "test"):
        ...
        mlflow_dataset = mlflow.data.from_pandas(
            split_frame,
            source=CodeDatasetSource({"uri": dataset.source_uri}),
            targets="label",
            name=f"kaggle-handwritten-digits-0-9-{split_name}",
...
def log_ray_task_timeline(run_id: str, ray_job_id: str) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(
        prefix="ray-handwritten-digits-timeline-check-"
