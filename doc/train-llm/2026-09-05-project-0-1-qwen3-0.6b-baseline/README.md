# 项目 0+1：环境/GPU 基线与 Qwen3-0.6B 基础推理

> 状态：方案已确认，当前只完成设计与运行契约；不启动训练、不修改外部数据。
>
> 对应路线图：[`2026-09-05-ai-girlfriend-learning-roadmap-design.md`](../2026-09-05-ai-girlfriend-learning-roadmap-design.md)

## 目标

把路线图中的“项目 0：环境与 GPU 基线”和“项目 1：0.6B 基础推理”合并为一个可独立验收的第一阶段。完成后应能从干净环境重复加载 `Qwen/Qwen3-0.6B`，对固定的 20 条脱敏上下文生成回复，并获得可比较的 GPU、延迟、吞吐和显存基线。

本阶段只回答两个问题：

1. 当前机器和项目环境能否稳定执行 BF16 的 0.6B 推理？
...
```text
数据根：/Users/weidian/project/luna-data/data-deal/output/wechat_aa807aaad90dc4463964/
dataset_id：wechat_aa807aaad90dc4464
...
- 候选计数为 3776/759/465；
- `datasets/train.jsonl`、`validation.jsonl`、`test.jsonl` 当前为空；
- 二次脱敏扫描和候选级泄漏检查通过；
- 人工审核尚未完成，正式 SFT 质量门禁仍为 blocked。

因此本阶段只从 `work/05_candidates/candidates.jsonl` 读取 validation session，生成 20 条固定推理 fixture。每条 fixture 的最后一个 `assistant` 目标回复会被移除；目标回复只作为受控的离线参考，不进入模型输入。
