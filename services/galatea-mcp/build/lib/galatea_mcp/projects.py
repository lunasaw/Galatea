    ...
    def verify(self, project_id, config_id, release_id, role):
        ...
        try:
            ...
            with zipfile.ZipFile(release_path) as archive:
                ...
                # Training/evaluation Releases must opt into the governed
                # Driver explicitly.  Additional arguments are immutable
                # Release material, never caller-controlled input.
                if release.entrypoint[1] == 'scripts/submit_train.py' and release.entrypoint[2:] != ['--run']:
                    raise ValueError('governed Driver mode required')
