...
本项目使用 Kaggle `olafkrastovski/handwritten-digits-0-9` 数据集，提供可恢复、可审计、
可由 Ray Job 提交的正式 PyTorch CUDA 13 十分类训练项目。它复用确定性数据切分、
MLflow 数据血缘和测试集门禁思想，并把配置、实现和入口分开。
...
当前阶段使用 Ray Train 做单 Worker 或多 Worker 数据并行训练。输入先灰度缩放、归一化并反相为黑底白笔画，卷积特征保留 4x4 空间网格。MLflow 的权威写入者始终
...
`mlflow.tracking_uri`；Trace 和 Run 都写入同一 `ray-handwritten-digits` Experiment。
...
```text
train-model/ray-handwritten-digits/
├── README.md
...
conda env create --file train-model/ray-handwritten-digits/conda.yaml
conda activate ray-handwritten-digits-py312
python -m pip install -e train-model/ray-handwritten-digits
python -m ipykernel install --user \
  --name ray-handwritten-digits-py312 \
  --display-name "Python (ray-handwritten-digits)"
python -m pip check
```
...
```text
/data/ai/chenzhangyue/code/data/handwritten-digits-kaggle/images/
├── digit_0/
├── digit_1/
└── ... digit_9/
```
...
`platform-data/ray-handwritten-digits/manifests/`，随后通过 MLflow Artifact API 保存。

如果训练节点读取的是某个已校验的 MinIO 数据快照，可声明真实来源 URI；不要把本地缓存
伪装成未核对的对象版本：

```bash
export HANDWRITTEN_DIGITS_DATASET_SOURCE_URI=\
kaggle://olafkrastovski/handwritten-digits-0-9
```

`HANDWRITTEN_DIGITS_DATASET_SOURCE_URI` 只记录经过核对的来源血缘；当前数据加载仍读取
`data.root` 的本地只读解压目录。多节点训练必须让所有解码节点以相同绝对路径只读挂载同一快照，
或先实现并验证真正的远程 Datasource。
...
```bash
python train-model/ray-handwritten-digits/scripts/train.py \
  --config train-model/ray-handwritten-digits/configs/smoke.yaml \
  --check-config
```
...
```bash
python train-model/ray-handwritten-digits/scripts/train.py \
  --config train-model/ray-handwritten-digits/configs/baseline.yaml
```
...
```bash
python train-model/ray-handwritten-digits/scripts/train.py \
  --config train-model/ray-handwritten-digits/configs/distributed.yaml
```
...
ray job submit \
  ...
  --runtime-env-json='{"working_dir":"train-model/ray-handwritten-digits","py_modules":["train-model/ray-handwritten-digits/src/ray_handwritten_digits"],"excludes":["notebooks/**","tests/**"]}' \
  -- python scripts/train.py --config configs/smoke.yaml
```

Notebook 提交使用同一个 `build_runtime_env(PROJECT_ROOT)`：`working_dir` 上传配置和入口，
`py_modules` 把真正的包目录 `src/ray_handwritten_digits` 加入 Python 导入路径。Driver 会把上传后
...
每个 epoch 的 `train.report` 同时写入 MLflow，包括 loss、accuracy、各类别与 macro
...
```bash
python train-model/ray-handwritten-digits/scripts/train.py \
  --config train-model/ray-handwritten-digits/configs/champion.yaml
```
...
如果 Driver 能导入 `ray_handwritten_digits`，但日志中的 `TrainController` 或 Worker 报
`ModuleNotFoundError: No module named 'ray_handwritten_digits'`，先确认提交参数同时包含项目
