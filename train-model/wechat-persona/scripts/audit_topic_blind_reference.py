#!/usr/bin/env python3
"""Plan, execute or replay a two-stage independent machine reference."""
import argparse
import json
import os
...
from wechat_persona.topic_blind_reference import run_reference


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--scope', choices=['preflight', 'private'], required=True)
    parser.add_argument('--config', dest='config_path', type=Path, default=ROOT / 'configs/topic-blind-reference-v1.yaml')
    for name in ('output-root', 'controlled-root'):
        parser.add_argument('--' + name, type=Path, required=True)
    for name in ('packet', 'consent', 'preflight'):
