...
from ray_handwritten_digits.data import (
    PreparedDataset,
    build_ray_datasets,
    prepare_dataset,
    validate_equal_shards,
)
from ray_handwritten_digits.evaluate import evaluate_checkpoint
from ray_handwritten_digits.integrity import build_integrity_report, integrity_digest
from ray_handwritten_digits.runtime import (
    controller_pickle_by_value,
    execution_provenance,
    ray_init_runtime_env,
    worker_runtime_env,
)
from ray_handwritten_digits.tracking import (
    RayMlflowCallback,
    code_identity,
    idempotency_key,
    inspect_tracking,
...
from ray_handwritten_digits.worker import train_loop_per_worker
...
def _log_checkpoint_and_selection(
    ...
    with tempfile.TemporaryDirectory(
        prefix="ray-handwritten-digits-checkpoint-check-"
...
def _log_mlflow_model(
    ...
    with checkpoint.as_directory() as checkpoint_directory:
        ...
        model.eval()
        input_example = np.zeros((2, 1, *config.image_size), dtype="float32")
        with torch.no_grad():
        ...
        model_info = mlflow.pytorch.log_model(
            ...
            metadata={
                "class_names": [f"digit_{index}" for index in range(10)],
    ...
    with tempfile.TemporaryDirectory(
        prefix="ray-handwritten-digits-model-check-"
    ) as verification_directory:
        downloaded_model = mlflow.artifacts.download_artifacts(
    ...
    with tempfile.TemporaryDirectory(
        prefix="ray-handwritten-digits-run-model-check-"
    ) as verification_directory:
...
@mlflow.trace(name="ray_handwritten_digits_training", span_type=SpanType.WORKFLOW)
def run_training(config: ProjectConfig, *, force: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        with mlflow.start_run(
            ...
            tags=tags,
            description=(
                "Ray Train PyTorch CUDA 13 handwritten-digits workload with deterministic "
            ...
            try:
                ...
                with tempfile.TemporaryDirectory(
                    prefix="ray-handwritten-digits-artifact-check-"
                ) as verification_directory:
                    verify_artifact_round_trip(
